...
def _build_chrome(project):
    ...
    theme = (
        ...
        .only("primary_color", "project_id", "category_above_hero",
              ...
              "homepage_sections", "products_per_row", "products_shown", "section_titles",
              "hide_featured_eyebrow", "category_mobile_slider")
        .first()
    )
    ...
    return {
        ...
        # Blank is heading_align's own "use each heading's own design" sentinel
        # (some headings are center-aligned on purpose) — must not be coerced
        # to "left" here, unlike heading_size/heading_font, whose defaults
        # ("md"/"display") already *are* the "keep original" sentinel.
        ...
        "products_shown": getattr(theme, "products_shown", 8) or 8,
        ...
        "category_mobile_slider": bool(getattr(theme, "category_mobile_slider", False)),
